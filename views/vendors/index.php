...
<div class="flex justify-between items-center mb-4">
    <div>
        <h1 class="text-2xl font-bold text-slate-900">Vendors & Suppliers</h1>
        <p class="text-sm text-slate-500 mt-0.5">Manage your supplier database and payables</p>
    </div>
    <a href="<?= BASE_URL ?? '' ?>/vendors/create" class="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-lg font-medium shadow-sm shadow-primary/30 hover:bg-primary/90 hover:shadow-md hover:shadow-primary/40 active:scale-95 transition-all text-sm">
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
    <div class="bg-white p-4 rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-shadow group">
        <div class="flex justify-between items-start">
            <div>
                <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-500 mb-0.5">Total Vendors</p>
    ...
    <div class="bg-white p-4 rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-shadow group">
        <div class="flex justify-between items-start">
            <div>
                <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-500 mb-0.5">Active Vendors</p>
    ...
    <form method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
        <div class="md:col-span-4">
            ...
            <div class="relative">
                ...
                <input type="text" name="search" class="w-full pl-9 pr-4 py-1.5 bg-slate-50 border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all" placeholder="Search by name, code, email..." value="<?= htmlspecialchars($_GET['search'] ?? '') ?>">
            </div>
        </div>
        ...
        <div class="md:col-span-3 flex gap-2">
            ...
            <a href="<?= BASE_URL ?? '' ?>/vendors" class="flex items-center justify-center px-4 py-1.5 bg-slate-100 text-slate-600 rounded-md text-sm font-medium hover:bg-slate-200 transition-colors" title="Clear Filters">
        ...
        <table class="w-full text-sm text-left">
            <thead class="bg-slate-50 text-slate-500 text-xs uppercase tracking-wider font-semibold border-b border-slate-200">
                <tr>
                    ...
                    <th class="px-4 py-2 text-center whitespace-nowrap">Total POs</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr>
                    <td colspan="8">
                        <div class="flex flex-col items-center justify-center py-12 text-slate-400">
                            <span class="material-symbols-outlined text-4xl mb-3 opacity-50">storefront</span>
                            <p class="text-base font-medium text-slate-500">No vendors found</p>
                            <p class="text-sm mt-1">Adjust your filters or add a new vendor.</p>
                        </div>
                    </td>
                </tr>
                <?php else: ?>
                ...
                <tr class="hover:bg-slate-50/80 transition-colors group">
                    <td class="px-4 py-1.5 whitespace-nowrap">
                        <a href="<?= BASE_URL ?? '' ?>/vendors/view/<?= $vendor['id'] ?>" class="font-medium text-primary hover:text-primary/80 transition-colors">
                            <?= htmlspecialchars($vendor['vendor_code']) ?>
                        </a>
                    </td>
                    <td class="px-4 py-1.5 whitespace-nowrap max-w-[200px] truncate" title="<?= htmlspecialchars($vendor['company_name']) ?>">
                        <div class="font-medium text-slate-900"><?= htmlspecialchars($vendor['company_name']) ?></div>
                        <div class="text-xs text-slate-500"><?= htmlspecialchars($vendor['contact_person']) ?></div>
                    </td>
                    <td class="px-4 py-1.5 text-slate-600 whitespace-nowrap">
                        <?= htmlspecialchars($vendor['category_name'] ?? 'Uncategorized') ?>
                    </td>
                    <td class="px-4 py-1.5 whitespace-nowrap">
                        <div class="text-slate-900"><?= htmlspecialchars($vendor['email']) ?></div>
                        <div class="text-xs text-slate-500"><?= htmlspecialchars($vendor['phone'] ?: 'No phone') ?></div>
                    </td>
                    <td class="px-4 py-1.5 text-center whitespace-nowrap">
                        <span class="inline-flex items-center justify-center px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-xs font-semibold">
                            <?= number_format($vendor['total_pos']) ?>
                        </span>
                    </td>
                    <td class="px-4 py-1.5 text-right whitespace-nowrap">
                        <span class="font-medium <?= ($vendor['outstanding_balance'] ?? 0) > 0 ? 'text-red-500' : 'text-green-600' ?>">
                            <?= htmlspecialchars($currency) ?> <?= number_format($vendor['outstanding_balance'] ?? 0, 2) ?>
                        </span>
                    </td>
                    <td class="px-4 py-1.5 text-center whitespace-nowrap">
                    ...
                    <td class="px-4 py-1.5 text-center whitespace-nowrap">
                        <div class="flex items-center justify-center gap-2">
                            <a href="<?= BASE_URL ?? '' ?>/vendors/view/<?= $vendor['id'] ?>" class="h-7 w-7 rounded border border-slate-200 text-slate-500 flex items-center justify-center hover:bg-slate-50 hover:text-primary transition-colors shadow-sm" title="View">
                                <span class="material-symbols-outlined text-sm">visibility</span>
                            </a>
                            <a href="<?= BASE_URL ?? '' ?>/vendors/edit/<?= $vendor['id'] ?>" class="h-7 w-7 rounded border border-slate-200 text-slate-500 flex items-center justify-center hover:bg-slate-50 hover:text-blue-600 transition-colors shadow-sm" title="Edit">
                                <span class="material-symbols-outlined text-sm">edit</span>
                            </a>
                            <a href="<?= BASE_URL ?? '' ?>/purchase-orders/create?vendor_id=<?= $vendor['id'] ?>" class="h-7 w-7 rounded border border-slate-200 text-slate-500 flex items-center justify-center hover:bg-slate-50 hover:text-green-600 transition-colors shadow-sm" title="New Purchase Order">
